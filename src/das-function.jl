...
function _check_das_params(σ::T, αL::T, nL::T, αR::T) where {T <: Real}
...
struct DasFunction{T <: Real} <: ContinuousUnivariateDistribution
    ...
    function DasFunction(μ::T, σ::T, ψ::T, αL::T, nL::T, αR::T) where {T <: Real}
        _check_das_params(σ, αL, nL, αR)
        ...
        new{T}(μ, σ, ψ, αL, αR, nL, σL, σR, xL, xR, NL, G_xL, G_xR, L_xL, L_xR, norm_const, p_xL, p_xR, p_mu)
    end
end

function Distributions.pdf(d::DasFunction{T}, x::Real) where {T <: Real}
    ...
    # right tail
    return d.norm_const * d.G_xR * exp(d.L_xR * (x - d.xR))
end

function Distributions.cdf(d::DasFunction{T}, x::Real) where {T <: Real}
    BifGauss = BifurcatedGaussian(d.μ, d.σ, d.ψ)
    const_left = d.G_xL / d.L_xL * d.NL / (d.NL - 1)
    const_right = - d.G_xR / d.L_xR
    ...
    elseif x >= d.xL && x <= d.xR
        ...
        return d.p_xR + d.norm_const * (- const_right * exp(d.L_xR * (x - d.xR)) + const_right)
    end
end


function Distributions.quantile(d::DasFunction{T}, p::Real) where {T <: Real}
    if p < zero(T) || p > one(T)
    ...
    elseif p > d.p_mu && p <= d.p_xR
        ...
        return d.xR + 1 / d.L_xR * log(d.L_xR / d.G_xR * ((p - d.p_xR) / d.norm_const - const_right))
    end
end

Distributions.maximum(d::DasFunction{T}) where {T <: Real} = T(Inf)
Distributions.minimum(d::DasFunction{T}) where {T <: Real} = T(-Inf)