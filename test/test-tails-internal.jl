...
@testset "CrystalBallTail integral functions" verbose = true begin
    ...
    # Test 3: Error for L <= 0
    @testset "Error handling for L <= 0" begin
        # Create a tail with L_x0 <= 0 (right tail)
        right_tail = DistributionsHEP.CrystalBallTail(1.0, 3.2, -2.0, 2.0)  # L_x0 = -2.0 < 0
        
        @test_throws ErrorException _integral(right_tail, -1.0)
        @test_throws ErrorException _integral_inversion(right_tail, 0.5)
    end
end
