...
@testset "HyperbolicSecant Distribution" verbose = true begin
    @testset "Construction" begin
        ...
        @test_throws ArgumentError HyperbolicSecant(0.0, 0.0)  # σ must be positive
        @test_throws ArgumentError HyperbolicSecant(0.0, -1.0)  # σ must be positive
    end

    @testset "Support and Properties" begin
        # Test support
        @test support(d_standard) == RealInterval(-Inf, Inf)
        @test support(d_general) == RealInterval(-Inf, Inf)
    ...
    @testset "CDF and Quantile" begin
        ...
        @test quantile(d_standard, 0.5) ≈ 0.0
    ...
    @testset "Special Functions" begin
        ...
        for t in [0.1, 0.5, 1.0]
            @test mgf(d, t) ≈ sec(t)  # For standard case μ=0, σ=1
            @test mgf(d, -t) ≈ sec(t)  # Should be even function for μ=0
        end
        
        # Test that MGF throws error for large t
        @test_throws DomainError mgf(d, 2.0)  # |t| >= π/2
        @test_throws DomainError mgf(d, -2.0)  # |t| >= π/2
        ...
        @test real(cf(d, 1.0)) ≈ sech(1.0)
        @test imag(cf(d, 1.0)) ≈ 0.0 atol=1e-15
    end
    ...
    @testset "Edge Cases" begin
        ...
        @test_throws BoundsError quantile(d_standard, 0.0)
        @test_throws BoundsError quantile(d_standard, 1.0)
