...
@testset "DoublesidedBifurcatedCrystalBall Distribution" verbose = true begin
    ...
    @testset "Quantile properties" begin
        ...
        for x in [-3.0, -2.0, -1.0, -0.5, 0.0, 0.5, 1.0, 2.0, 3.0, 4.0]
            p = cdf(d, x)
    ...
    @testset "Sigma scaling" begin
        ...
        test_cases = [
            (0.0, 0.5, 0.25, 0.5, 1.25, 0.75, 1.5),  # σ < 1
            (0.0, 1.0, 0.25, 0.5, 1.25, 0.75, 1.5),  # σ = 1
            (0.0, 5.0, 0.25, 0.5, 1.25, 0.75, 1.5),  # σ > 1
        ]
